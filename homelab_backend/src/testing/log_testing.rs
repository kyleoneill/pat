...
mod log_testing {
    ...
    async fn log_generation() {
        ...
        let user = get_user_me(&helper.client, token.as_str(), &helper.address)
            .await
            .unwrap();
        get_user_me(&helper.client, token.as_str(), &helper.address)
            .await
            .unwrap();
        ...
        // Get logs
        let logs = get_logs_for_user(&helper.client, token.as_str(), &helper.address)
            .await
            .unwrap();
        ...
        .await
        .unwrap();
        let user_two = get_user_me(&helper.client, token_two.as_str(), &helper.address)
            .await
            .unwrap();
        ...
        let logs = get_logs_for_user(&helper.client, token_two.as_str(), &helper.address)
            .await
            .unwrap();
