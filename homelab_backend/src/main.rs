...
use api::{chat_controller, games_controller, log_controller, reminder_controller, user_controller};
    ...
    let trace_layer = TraceLayer::new_for_http()
        ...
        .on_request(move |request: &Request<_>, _span: &Span| {
            if LOGGABLE_METHODS.contains(request.method()) {
                ...
                let new_task =
                    tasks::log_creation_task::LogCreationTask::new(request.method().to_string(), request.uri().to_string(), user_id, date_time);
