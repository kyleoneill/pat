...
case user_crud() {
    ...
    var token = POST /api/users/auth username=(username) password=(password);
    DELETE /api/users/me authorization:(token.body);

    case create_user_bad_args() {
        // Try to create a user with no args
        var noArgs = POST /api/users;
        ASSERT STATUS (noArgs) 422;

        // Try to create a user with bad args
        var badArgs = POST /api/users foo=5 bar="foobar";
        ASSERT STATUS (badArgs) 422;

        // Try to create a user without a password
        var noPassword = POST /api/users username=(username);
        ASSERT STATUS (noPassword) 422;
    }

    // Create a user
    var user = POST /api/users username=(username) password=(password);
    ASSERT STATUS (user) 201;
    ASSERT EQUALS (user.body.username) (username);

    // Try to create the user again, assert we get an error
    var again = POST /api/users username=(username) password=(password);
    ...
    // Get a user
    var user = GET /api/users/me authorization:(token.body);
    ...
    case user_permissions() {
         case other_user_permissions() {
             ...
             POST /api/users username="dummy" password="dummy";

             // Auth as a second user
             var authUserTwo = POST /api/users/auth username="dummy" password="dummy";
             ASSERT STATUS (authUserTwo) 201;

             // Get the first user as the second
             var otherUser = GET /api/users?username=(username) authorization:(authUserTwo.body);
             ASSERT STATUS (otherUser) 200;

             // TODO Update other, fail

             // Try to delete another user
             var deleteOther = DELETE /api/users/(otherUser.body.id) authorization:(authUserTwo.body);
             ASSERT STATUS (deleteOther) 403 "Failed to assert that one user cannot delete another user";
         }
         case admin_user_permissions() {
            // Auth as an admin
            var adminRes = POST /api/users/auth username="admin" password="test";
            ASSERT STATUS (adminRes) 201;
         }
    }

    // Delete the user
    var deleteRes = DELETE /api/users/me authorization:(token.body);
...
case auth_user() {
    ...
    // Auth as the user
    var res = POST /api/users/auth username=(username) password=(password);
    ASSERT STATUS (res) 201;

    // Make a request with valid auth
    var goodAuth = GET /api/users/me authorization:(res.body);
    ASSERT STATUS (goodAuth) 200;

    // Make a request with invalid auth
    var badAuth = GET /api/users/me authorization:"InvalidTokenString";
    ASSERT STATUS (badAuth) 404;

    // Auth as the user with an invalid password
    var badPasswordRes = POST /api/users/auth username=(username) password="wrongPassword";
    ASSERT STATUS (badPasswordRes) 404;

    // Auth as a user that does not exist
    var badRes = POST /api/users/auth username="idontexist" password=(password);
